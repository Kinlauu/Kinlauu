...
def format_text(all_text):
    ...
    for line in lines:
        ...
        line_clean = re.sub(r'2020-2021年度濠江中學推.*?[保筆].*?問卷', '', line_clean)
        line_clean = re.sub(r'填寫日期[：:]?', '', line_clean)
        
        # Additional cleanups just in case
        line_clean = line_clean.strip()
        if not line_clean:
            continue
            
        # Match University
        univ_match = re.search(r'(报考院校|录取院校|学校|院校名称)[:：]?\s*(.*?大学|.*?学院)', line_clean)
        if univ_match:
        ...
        # Match Major
        major_match = re.search(r'(专业|录取专业|报考专业|专业名称)[:：]?\s*(.*)', line_clean)
        if major_match:
            major = major_match.group(2).strip()
            # If the regex matched but major is empty, skip grabbing the empty one
            if major:
                # Remove name from the major string just in case
        ...
        # Check Exam Format
        exam_format_match = re.search(r'(考试形式|测试形式|面试形式)[:：]?(.*)', line_clean)
        if exam_format_match:
            fmt = exam_format_match.group(2).strip()
            formatted_md += f"考试形式：{fmt}\n"
            continue
...
if __name__ == "__main__":
    ...
    md_path = os.path.join(base_dir, "20-21联校保送问卷_重制排版.md")
